...
public class FileListDoubleSender extends Sender implements Callable {
	...
	public Object call() throws Exception {
		...
		if (target != null) {
			...
			ResultSet rs = null;
			// 根据双列文件信息形成获取双列文件信息列表的SQL语句
			String sql = "select MobilePhone,SMContent from avs_txl_mobilelist2 where pihao='"
					+ target + "'";
			try {
				...
				Log.LogInfo(FileListDoubleSender.class + ".call方法错误！");
			} finally {
	...
	private void subHandleSendTarget(ResultSet rs) {
		...
		try {
			...
			sendTarget.setSMContent(rs.getString("SMContent"));
