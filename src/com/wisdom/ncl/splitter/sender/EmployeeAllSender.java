...
public class EmployeeAllSender extends Sender implements Callable {
		...
		if (m_send_sm != null) {
			...
			if (target != null) {
				...
				try {
					...
					Log.LogInfo(EmployeeAllSender.class + ".call方法错误！");
				} finally {
	...
	private void subHandleSendTarget(ResultSet rs) {
		...
		try {
			...
			sendTarget.setSendTargetName(rs.getString("EmployeeName"));
		} catch (SQLException e) {
			Log.LogInfo(EmployeeAllSender.class + ".subHandleSendTarget方法错误！");
		}
